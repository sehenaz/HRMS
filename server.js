...
// ════════════════════════════════════════════════════════
//
// FIX: previously a plain mongoose.connect(...).then().catch() was used.
// If the connection later dropped (laptop sleep, WiFi blip, Atlas hiccup),
// Mongoose would NOT automatically re-establish it in a way the driver's
// buffered queries could recover from in time — every subsequent query
// would fail with "Operation `...` buffering timed out after 10000ms"
// until the process was manually restarted.
//
